...
// Dans le composant VoitureMarqueSuite
const { idMarque } = useParams<{ idMarque: string }>();

  const apiUrl = 'https://cloud-back-voiture-production.up.railway.app/annonce/nouvelle_annonce'; // Remplace TON_URL_API par ton URL réelle
  ...
  console.log("history ito  = " ,history );
    ...
    e.preventDefault();
     ...
     try {
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({immatriculation , kilometre, prix, annee,description , modele}),
      ...
      if (response.ok) {
        ...
        const allModele: Modele[] = data.object.map((model: any) => ({
          _id: model._id,
          nom: model.nom

        }));

        console.log("allmodele = ", allModele);
        setModele(allModele);
...
<form onSubmit={handleRequest}>
  ...
  <input className='input' type="text" placeholder='immatriculation' style={{marginTop:'110px'}} 
    value={immatriculation}
    onChange={(e) => setImmatricul(e.target.value)} />

  <input className='input' type="text" style={{}} placeholder="kilometre"
    value={kilometre}
  ...
  <input className='input' type="text" placeholder='Année' style={{}}
    value={annee}
  ...
  <select className='select' style={{}} name='modele' >
    {modele.map((model) => (
